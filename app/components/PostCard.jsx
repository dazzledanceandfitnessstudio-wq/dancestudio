...
function getExcerpt(post) {
  // 🔥 FIX: Pehle description check karo (kyunki aapke database mein yeh field hai)
  if (post.description) {
    return post.description;
  }
  
  // Agar description nahi to excerpt check karo
  if (post.excerpt) {
    return post.excerpt;
  }
  
  // Agar excerpt nahi to content se banao
  if (post.content) {
    const plain = post.content.replace(/<[^>]*>/g, "");
    return plain.length > 200 ? plain.slice(0, 200).trim() + "…" : plain;
  }
  
  // Agar kuch nahi to fallback message
...
export default function PostCard({ post, isFeatured = false }) {
  ...
  const excerptLines = isFeatured ? 4 : 3;
...
// function getExcerpt(post) {
//   // Debug
//   console.log("Getting excerpt for post:", post?.title);
  
//   if (post.excerpt) {
//     console.log("Using custom excerpt");
//     return post.excerpt;
//   }
  
//   if (post.content) {
//     console.log("Using content for excerpt");
//     const plain = post.content.replace(/<[^>]*>/g, "");
//     console.log("Plain text:", plain.substring(0, 50) + "...");
//     return plain.length > 200 ? plain.slice(0, 200).trim() + "…" : plain;
//   }
  
//   console.log("No excerpt or content found");
...
//           margin: 0,
//           flex: 1,
//           minHeight: "3rem", // Ensure space even if empty
//         }}>
...
//   const titleSize = isFeatured ? "1.5rem" : "1.125rem";
//   const excerptSize = isFeatured ? "1rem" : "0.875rem";
//   const excerptLines = isFeatured ? 4 : 3; // Increased lines
