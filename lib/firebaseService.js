...
export const authService = {
  signInWithGoogle: async () => {
    try {
      ...
      // ─── 📧 SEND EMAIL VIA API (BOTH NEW & EXISTING) ───
      console.log('📧 [Email] Sending email via API to:', user.email);
      try {
        const emailType = isNewUser ? 'welcome' : 'login';
        ...
        if (emailResponse.ok) {
          console.log(`✅ [Email] ${isNewUser ? 'Welcome' : 'Login'} email sent`);
        } else {
      ...
      // ─── 🔔 SEND ADMIN FCM (BOTH NEW & EXISTING) ───
      // 🔥 Yeh notification admin ke Flutter app mein jayega
      ...
      try {
        ...
        const notificationBody = isNewUser
          ? `${user.displayName || 'A new dancer'} (${user.email}) has joined Dazzle!`
          : `${user.displayName || 'A dancer'} (${user.email}) just logged in!`;
        ...
        const fcmResponse = await fetch('/api/fcm', {
          ...
          body: JSON.stringify({
            ...
            data: {
              type: isNewUser ? 'NEW_USER' : 'USER_LOGIN',
              userId: user.uid,
              name: user.displayName || 'User',
        ...
        console.log('🔔 [FCM] Admin notification response:', fcmData);
        
        if (fcmResponse.ok) {
          console.log('✅ [FCM] Admin notification sent successfully');
        } else {
          console.error('❌ [FCM] Admin notification failed:', fcmData);
        }
      } catch (fcmError) {
        console.error('❌ [FCM] Exception:', fcmError.message);
      }

      // ─── 🔥 FCM TOKEN REGISTRATION - SIRF FLUTTER APP KE LIYE ───
      // 🔥 CHECK: Agar browser hai toh skip karo
      const isBrowser = typeof window !== 'undefined';
      const isFlutterApp = isBrowser && navigator?.userAgent?.includes('Flutter');
      
      if (isBrowser && !isFlutterApp) {
        console.warn('⚠️ [FCM] Web browser detected. FCM token registration only for Flutter app. Skipping...');
      } else {
        console.log('🔔 [FCM] Registering/Updating FCM token for user:', user.email);
        try {
          ...
          if (isBrowser && 'Notification' in window) {
            if (Notification.permission === 'denied') {
              ...
              if (permission === 'granted') {
                ...
                if (fcmToken) {
                  const tokenResponse = await fetch('/api/fcm', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      token: fcmToken,
                  ...
                  console.log('🔔 [FCM] Token registration response:', tokenData);
                  
                  if (tokenResponse.ok) {
                    console.log('✅ [FCM] Token registered/updated successfully');
                  } else {
                    console.error('❌ [FCM] Token registration failed:', tokenData);
                  }
                } else {
            ...
            console.warn('⚠️ [FCM] Notification API not available');
          }
        } catch (fcmError) {
          console.warn('⚠️ [FCM] Could not register token:', fcmError.message);
        }
      }
...
export const enrollmentService = {
  requestEnrollment: async (userId, eventId) => {
    try {
      ...
      // ─── 📧 SEND ENROLLMENT REQUEST EMAIL VIA API ───
      ...
      try {
        ...
        if (emailResponse.ok) {
          console.log('✅ [Email] Enrollment request email sent');
        } else {
          console.error('❌ [Email] Failed to send:', emailData);
        }
      } catch (emailError) {
...
//       // ─── 🔔 REGISTER/UPDATE FCM TOKEN (BOTH NEW & EXISTING) ───
//       console.log('🔔 [FCM] Registering/Updating FCM token for user:', user.email);
//       try {
//         const { getMessaging, getToken } = await import('firebase/messaging');
//         const messaging = getMessaging();
