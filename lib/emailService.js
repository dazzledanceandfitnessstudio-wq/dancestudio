...
export const emailService = {
  sendMailByApp: async (email, subject, html, buttonText = null, buttonLink = null, isSuccess = true) => {
    if (typeof window !== 'undefined') {
      console.warn('⚠️ sendMailByApp called on client-side');
  ...
  sendWelcomeEmail: async (email, name) => {
    ...
    try {
      const content = `
        ...
        <p style="margin-top: 16px;">Keep dancing,<br><strong>Team Dazzle</strong> 💫</p>
      `;

      return await emailService.sendMailByApp(
        email,
        '🎉 Welcome to Dazzle Dance Studio!',
        content,
        'Explore Dashboard',
        `${APP_URL}/dashboard`,
        true
  ...
  sendLoginEmail: async (email, name) => {
    if (typeof window !== 'undefined') {
      return { success: false, error: 'Email can only be sent from server' };
    }
    ...
    try {
      const content = `
        <p>Hello <strong>${name || 'Dancer'}</strong>! 👋</p>
        <p>Great to see you again at <strong>Dazzle Dance Studio</strong>! 🎊</p>
        <div class="highlight" style="border-left-color: #4A90D9;">
          <p style="margin-bottom: 0;">
            <strong>📅 Ready for your next dance session?</strong><br>
            Check out our latest events and classes waiting for you!
          </p>
        </div>
        <p>🎯 <strong>Quick Links:</strong></p>
        <ul style="color: #555; line-height: 2; padding-left: 20px;">
          <li>📋 View your <strong>enrolled events</strong></li>
          <li>🎵 Discover <strong>new classes</strong></li>
          <li>💬 Connect with the <strong>dance community</strong></li>
        </ul>
        <p>Keep dancing! 💃</p>
        <p style="margin-top: 16px;"><strong>Team Dazzle</strong> 💫</p>
      `;

      return await emailService.sendMailByApp(
        email,
        '👋 Welcome Back to Dazzle Dance Studio!',
        content,
        'Go to Dashboard',
        `${APP_URL}/dashboard`,
        true
  ...
  sendContactFormEmail: async (name, email, phone, subject, message) => {
    if (typeof window !== 'undefined') {
      return { success: false, error: 'Email can only be sent from server' };
    }
    ...
    try {
      const adminContent = `
        ...
        <div class="highlight">
          <p style="margin-bottom: 0;">
            ...
            <strong>Email:</strong> <a href="mailto:${email}" style="color: #FF1F6D;">${email}</a><br>
            <strong>Phone:</strong> ${phone || 'Not provided'}<br>
            <strong>Subject:</strong> ${subject || 'General Inquiry'}
          </p>
        </div>
        <p><strong>Message:</strong></p>
        <p style="background: #f8f5ff; padding: 16px; border-radius: 12px; color: #333; line-height: 1.8;">
          ${message}
        </p>
        <p style="margin-top: 16px; font-size: 14px; color: #8c82a0;">
          Reply directly to this email to respond to the user.
        </p>
      `;

      await emailService.sendMailByApp(
        EMAIL_USER,
      ...
      const userContent = `
        <p>Hello <strong>${name || 'Dancer'}</strong>! 👋</p>
        <p>Thank you for reaching out to <strong>Dazzle Dance Studio</strong>! 🎊</p>
        <div class="highlight">
          <p style="margin-bottom: 0;">
        ...
        <p style="margin-top: 16px;">
          In the meantime, feel free to explore our events and classes!<br>
